...
class SettingsSchemaMetadata:
    ...
    # Standard JSON Schema for the settings type, generated by pydantic when
    # the optional `schema` extra is installed (None otherwise, and for types
    # pydantic cannot model). Rides everywhere this schema already goes —
    # component metadata, settings snapshots, settings events — so any
    # consumer can validate values without parsing the field_type strings.
