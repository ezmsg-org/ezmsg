...
def setup_graph_and_shm(msg_size):
    ...
    with MessageMarshal.serialize(0, aa) as (size, _, _):
        msg_size_bytes = size
    shm = loop.run_until_complete(
        graph_service.create_shm(num_buffers=32, buf_size=msg_size_bytes + 100)
    )
...
def main():
    ...
    for msg_size in msg_sizes:
        ...
        recon_time = (
            timeit.timeit(stmt=lambda: recon_without_deepcopy_func(shm), number=ITERS)
            / ITERS
            * 1e9
        )  # ns
        deepcopy_time = (
            timeit.timeit(stmt=lambda: recon_with_deepcopy_func(shm), number=ITERS)
            / ITERS
            * 1e9
        )  # ns
