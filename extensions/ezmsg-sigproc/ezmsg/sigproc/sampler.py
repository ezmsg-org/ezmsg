...
class Sampler( ez.Unit ):
    ...
    async def on_trigger( self, msg: SampleTriggerMessage ) -> None:
        if self.STATE.last_msg is not None:
            ...
            if period is None:
                logger.warning( f'Sampling failed: period not specified' )
                return
            ...
            if ( stop_offset - start_offset ) <= 0:
                logger.warning( f'Sampling failed: invalid period requested' )
                return
            ...
            if req_buf_len >= max_buf_len:
                logger.warning( f'Sampling failed: {period=} >= {self.SETTINGS.buffer_dur=}' )
                return
            ...
            if -min( offset + start_offset, 0 ) >= self.STATE.buffer.shape[0]:
                logger.warning( 'Sampling failed: insufficient buffer accumulation for requested sample period' )
                return

            trigger = replace( msg, period = period, value = value )
            self.STATE.triggers[ trigger ] = offset 

        else: logger.warning( 'Sampling failed: no signal to sample yet' )
    ...
    async def on_signal( self, msg: TSMessage ) -> None:
        ...
        if ( # Check if signal properties have changed in a breaking way
            ...
            if len( self.STATE.triggers ) > 0:
                logger.warning( 'Sampling failed: Discarding all triggers' )
            logger.warning( 'Flushing buffer: signal properties changed' )
